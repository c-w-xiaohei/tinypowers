...
# 1. Create in Tinypowers location (lowest priority)
mkdir -p "$TINYPOWERS_SKILLS_DIR/priority-test"
cat > "$TINYPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Tinypowers version of priority test skill
---
# Priority Test Skill (Tinypowers Version)

This is the TINYPOWERS version of the priority test skill.

PRIORITY_MARKER_TINYPOWERS_VERSION
EOF
...
if [ -f "$TINYPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Tinypowers version exists"
else
    echo "  [FAIL] Tinypowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled Tinypowers priority
echo ""
echo "Test 2: Documenting personal vs Tinypowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled Tinypowers version instead of personal version"

# Test 3: Document project vs bundled Tinypowers priority
echo ""
echo "Test 3: Documenting project vs personal/Tinypowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled Tinypowers version instead of project version"

# Test 4: Test a non-colliding bundled Tinypowers skill is still available
echo ""
echo "Test 4: Testing non-colliding Tinypowers skill remains available..."

mkdir -p "$TINYPOWERS_SKILLS_DIR/tinypowers-only-test"
cat > "$TINYPOWERS_SKILLS_DIR/tinypowers-only-test/SKILL.md" <<'EOF'
---
name: tinypowers-only-test
description: Tinypowers-only priority test skill
---
# Tinypowers Only Test Skill

PRIORITY_MARKER_TINYPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"tinypowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_TINYPOWERS_ONLY_VERSION" "Non-colliding Tinypowers skill is still registered"
