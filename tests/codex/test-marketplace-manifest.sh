...
assert_equal(marketplace.get("name"), "tinypowers-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "tinypowers"]
assert_equal(len(matching_plugins), 1, "tinypowers plugin entry count")
